...
def algorithm(weight: list) ->float:
    ...
    #         {'type':'ineq', 'fun': con2}]
    # x = minimize(sharpe, weight, constraints=cons)
    #GA package minimizes
    return  (-1 *sharpe(weight) + penaltyCount(weight))
...
desired_expectancy_ratio = 0.3#realistic
#each should be [0,1]  Each adds extra terms
def con3(x:list) -> int:
    return abs(desired_expectancy_ratio - sum(np.multiply(x,st_means)) )

def penaltyCount(x):
    return (100*con1(x))**2 + 100*(con2(x)**2) 
itermax = 360
